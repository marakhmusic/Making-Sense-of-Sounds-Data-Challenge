...
DIRNAME = r'/home/hguan/Data-Challenge/Development'
...
DIRNAME_TEST = r'/home/hguan/Data-Challenge/Evaluation'
def get_test_file_paths(dirname):                        # function to access file in the directory
    file_paths = []
    # os.walk generates names in the
    for root_dirpath, directories, files in os.walk(dirname):
        for filename in files:                                            # directory tree, dirpath, dirnames, filenames
...
def _get_test_data():
  files        = get_file_paths(DIRNAME_TEST)  # function called to get the file paths
  File_names   = []
  all_examples = np.empty([0, 96, 64])
  for file in sorted(files):  # loop to access each file
    ...
    if ext == '.wav':
        ...
        examples_batch = vggish_input.waveform_to_examples(y,sr)
        all_examples   = np.concatenate((all_examples, examples_batch), axis = 0)


  number           = all_examples.shape[0] / 5
  
  csvfile = "file_order_1.csv"
...
def _get_examples_batch(integer):
  ...
  number           = all_examples.shape[0] / 5
  Effects_labels   = np.array([[1, 0, 0, 0, 0]] * int(number))
  Human_labels     = np.array([[0, 1, 0, 0, 0]] * int(number))
  Music_labels     = np.array([[0, 0, 1, 0, 0]] * int(number))
  Nature_labels    = np.array([[0, 0, 0, 1, 0]] * int(number))
  Urban_labels     = np.array([[0, 0, 0, 0 ,1]] * int(number))
  all_labels       = np.concatenate((Effects_labels, Human_labels, Music_labels, Nature_labels, Urban_labels))
...
def main(_):
  with tf.Graph().as_default(), tf.Session() as sess:
    ...
    test_features = _get_test_data()
    print(test_features.shape)

    num_epochs = 50
    for epoch_index in range(num_epochs):
      print("Now, Epoch: ", epoch_index + 1)
      for i in range(FLAGS.num_batches):
      #for i in range(1):
        (features, labels)   = _get_examples_batch(i)
        [num_steps, loss, _] = sess.run([global_step_tensor, loss_tensor, train_op],
                                        feed_dict = {features_tensor: features, labels_tensor: labels})
        
      print(loss)
    #predictions_tensor = sess.graph.get_operation_by_name('mymodel/test/test_op')
    prediction = tf.argmax(logits,1)
    best = sess.run([prediction],feed_dict={features_tensor: test_features})
    #predictions = sess.run(tf.convert_to_tensor(test_features, np.float32))
    
    result_for_each_file = [best[0][x:x + 5] for x in range(0, len(best[0]), 5)]
    print(len(result_for_each_file))
    final_results = []
    classes = ["Effects", "Human", "Music", "Nature", "Urban"]
    for cur_result in result_for_each_file:
        print('---------------')
        print(cur_result)
        class_index = max(Counter(cur_result).items(), key = operator.itemgetter(1))[0]
        print(classes[class_index])
        final_results.append(classes[class_index])

    csvfile = "out_Labels_1.csv"
    with open(csvfile, "w") as output:
      writer = csv.writer(output, lineterminator='\n')
      for val in result_for_each_file:
          writer.writerow([val]) 
